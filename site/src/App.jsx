Warning: truncated output (original token count: 42932)
...
function interpretMetric(label, context, kind) {
  ...
  if (label === "Cresc. receita" || label === "Cresc. lucro") {
    ...
    if (value === null) return assessment("n…12932 tokens truncated…7 24 meses", "Eventos oficiais B3", "dividendRegularity", [{ label: "Meses com provento", value: `${f.dividendMonths24m ?? 0} de 24` }]),
  ...
  return <div className={`modal-backdrop ${asPage ? "asset-page-backdrop" : ""}`} onMouseDown={(event) => !asPage && event.currentTarget === event.target && onClose()}>
    <section className="detail-sheet" role={asPage ? "region" : "dialog"} aria-modal={asPage ? undefined : "true"} aria-label={`An\xE1lise de ${asset.ticker}`}>
...
function OpportunityPage({ assets, onBack, onOpen }) {
  ...
  const unavailableValuations = useMemo(() => assets.filter((asset) => asset.fundamentals && !buildOpportunity(asset, assets)?.fair).length, [assets]);
...
function OptionsPage({ assets, anomalies, optionChain, onBack, onOpen }) {
  const [query, setQuery] = useState("");
  ...
  return <div className="options-page"><div className="daily-radar-top"><button className="back-btn" onClick={onBack}>← Visão geral</button><span>Camada educacional • sem lançamento descoberto</span></div>
    ...
    <OptionsLab assets={assets} anomalies={anomalies} optionChain={optionChain} />
    <section className="radar-method"><h2>Como usar a indicação de contrato?</h2><p>O ranking é uma seleção matemática para estudo, não uma ordem. Confirme a defasagem D-1, o spread na corretora, a liquidez atual e se a estratégia continua compatível com sua tese antes de qualquer decisão.</p><strong>Derivativos podem ampliar perdas. Defina limite de risco e nunca trate o Score do ativo-base como garantia para a opção.</strong><div className="integrity-sources"><a href="https://www.b3.com.br/pt_br/market-data-e-indices/servicos-de-dados/market-data/consultas/mercado-a-vista/opcoes/series-autorizadas/" target="_blank" rel="noreferrer">Séries autorizadas — B3 ↗</a><a href="https://www.gov.br/investidor/pt-br/investir/tipos-de-investimentos/derivativos/riscos" target="_blank" rel="noreferrer">Riscos dos derivativos — CVM ↗</a></div></section>
  </div>;
}
function Home() {
  ...
  useEffect(() => {
    const syncPage = () => setRoute(parseRoute(window.location.hash));
    window.addEventListener("hashchange", syncPage);
    return () => window.removeEventListener("hashchange", syncPage);
  }, []);
  useEffect(() => {
    if (page !== "asset") return;
    const asset = assets.find((item) => item.ticker === route.ticker);
    setSelected(asset ?? null);
  }, [assets, page, route.ticker]);
  useEffect(() => {
  ...
  const openPage = (next, ticker = null) => {
    const nextRoute = { page: next, ticker: ticker?.toUpperCase() ?? null };
    window.location.hash = routeHash(next, ticker);
    setRoute(nextRoute);
    if (next !== "asset") setSelected(null);
