...
export default function AppLite() {
  ...
  return <main className="lite-app">
    ...
    {page === "swing" && <div className="lite-page"><section className="lite-section-head swing-head"><div><span>RADAR SWING TRADE</span><h1>As 10 melhores para 1 a 6 meses</h1><p>Ranking quantitativo que combina fundamentos, valuation, momentum da janela escolhida, risco, liquidez e confiança dos dados. Uma empresa aparece apenas uma vez, mesmo quando possui mais de uma classe de ação.</p></div><label>Horizonte<select value={swingMonths} onChange={(event) => setSwingMonths(Number(event.target.value))}>{[1,2,3,4,5,6].map((month) => <option key={month} value={month}>{month} {month === 1 ? "mês" : "meses"}</option>)}</select></label></section><div className="swing-method-note"><b>Janela selecionada: {swingMonths} {swingMonths === 1 ? "mês" : "meses"}</b><span>Quanto menor a janela, maior o peso do timing. Quanto maior, maior o peso dos fundamentos. Ativos com cobertura inferior a 80% não entram no ranking.</span></div><section className="swing-top10-grid">{topSwing.map((row, index) => <button className="swing-pick-card" key={row.asset.ticker} onClick={() => setSelectedTicker(row.asset.ticker)}><div className="swing-rank"><span>#{index + 1}</span><em className={`swing-${row.signal}`}>{row.signal === "forte" ? "FORTE" : row.signal === "observar" ? "OBSERVAR" : "FRACO"}</em></div><div className="swing-name"><strong>{row.asset.ticker}</strong><small>{row.asset.name || row.asset.fundamentals?.companyName}</small></div><div className="swing-score"><b>{row.score}</b><span>/100</span></div><div className="swing-pick-metrics"><article><span>Fundamentos</span><b>{Math.round(row.fundamental)}</b></article><article><span>Momentum</span><b>{Math.round(row.momentum)}</b></article><article><span>Risco</span><b>{Math.round(row.risk)}</b></article><article><span>Liquidez</span><b>{Math.round(row.liquidity)}</b></article><article><span>Retorno janela</span><b>{pct(row.horizonReturnPct)}</b></article><article><span>RSI 14</span><b>{num(row.rsi14)}</b></article></div><footer><span>{money(row.asset.price)} · ref. {dateBR(row.technicalReferenceDate)}</span><b>Ver análise completa →</b></footer></button>)}</section>{!topSwing.length && <div className="lite-error">Ainda não há histórico técnico suficiente para montar o Top 10 desta janela. O ranking não inventa dados ausentes.</div>}</div>}
