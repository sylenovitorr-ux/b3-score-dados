...
export default function OptionsLab({ assets, anomalies, optionChain }) {
  ...
  return <section className="options-lab">
    ...
    <details className="option-market-import"><summary>Importar bid, ask e open interest <i>⌄</i></summary><div><div><b>Fotografia do mercado</b><p>Exporte ou monte um CSV com <code>ticker;bid;ask;openInterest;referenceDate;source</code>. O arquivo fica apenas neste dispositivo e nunca substitui os dados oficiais do contrato.</p></div><label className="option-file-button">Selecionar CSV<input type="file" accept=".csv,text/csv,text/plain" onChange={(event) => loadMarketFile(event.target.files?.[0])}/></label>{importReport && <p className={importReport.errors.length ? "import-errors" : "import-success"}>{importReport.rows.length} linhas válidas. {importReport.errors.length ? `${importReport.errors.length} rejeitadas: ${importReport.errors.slice(0, 2).join(" ")}` : "Validação concluída."}</p>}<small>{marketRows.length} contratos com fotografia armazenada.</small></div></details>
