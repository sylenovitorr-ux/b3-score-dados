...
function ClosedTradeEditor() {
  ...
  const save = () => {
    ...
    const updated = portfolios.map((portfolio) => portfolio.id !== selected.portfolioId ? portfolio : { ...portfolio, closedTrades: (portfolio.closedTrades ?? []).map((trade) => {
      ...
      next.audit = [...(trade.audit ?? []), { at: next.editedAt, type: "closed-trade-edit", reason: String(draft.editReason || "correção manual").trim(), before: { entryPrice: trade.entryPrice, exitPrice: trade.exitPrice, quantity: trade.quantity, entryDate: trade.entryDate, exitDate: trade.exitDate }, after: { entryPrice, exitPrice, quantity, entryDate: next.entryDate, exitDate: next.exitDate } }];
      return next;
    }) });
  ...
  return <section className="closed-editor"><header><span>OPERAÇÕES ENCERRADAS</span><h2>Edite sem apagar o passado.</h2><p>Qualquer correção guarda o valor anterior, data e motivo da alteração.</p></header>
    {trades.length ? <div className="closed-editor-list">{trades.map((row) => <article key={`${row.portfolioId}-${row.trade.id}`}><div><strong>{row.trade.ticker}</strong><small>{row.portfolioName}</small></div><span>Entrada<b>{money(row.trade.entryPrice)}</b><small>{row.trade.entryDate ?? "N/D"}</small></span><span>Saída<b>{money(row.trade.exitPrice)}</b><small>{row.trade.exitDate ?? "N/D"}</small></span><span>Resultado<b className={row.trade.returnPct == null ? "" : row.trade.returnPct >= 0 ? "positive" : "negative"}>{pct(row.trade.returnPct)}</b></span><button onClick={() => edit(row)}>Editar</button></article>)}</div> : <p className="portfolio-empty-state">Nenhuma posição encerrada ainda.</p>}
