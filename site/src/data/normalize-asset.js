...
export function normalizeAsset(raw) {
  ...
  return { ticker: String(raw.ticker ?? "").toUpperCase(), name: raw.name ? String(raw.name) : undefined, kind, date: raw.date ? String(raw.date) : undefined, price: numeric(raw.price), priceopen: numeric(raw.priceopen), high: numeric(raw.high), low: numeric(raw.low), volume: numeric(raw.volume), closeyest: numeric(raw.closeyest), change: numeric(raw.change), changepct: numeric(raw.changepct), high52: numeric(raw.high52), low52: numeric(raw.low52), tradetime: raw.tradetime ? String(raw.tradetime) : null, securityType: raw.securityType ? String(raw.securityType) : undefined, unitComposition: raw.unitComposition ? String(raw.unitComposition) : null, sector: raw.sector ? String(raw.sector) : null, segment: raw.segment ? String(raw.segment) : null, group: raw.group ? String(raw.group) : null, fundamentals, fund };
}
